...
class SimpleXLSX {
	...
	function sheetName( $id ) {
		if(!isset($this->workbook) || !isset($this->sheets) || !isset($this->sheet))
			return false;
	...
	function value( $cell ) {
		...
		switch ($dataType) {
			...
			default:
				...
				if(($numStyle = $cell['s']) == null)
					$value = (string)$cell->v;
 				else{
					$style = $this->styles->cellXfs->xf[intval($numStyle)]['numFmtId'];
						switch ($style) {
							case "0":
									$value = $cell->v;
								break;
							case "1":
									$value = (integer)$cell->v;
								break;
							case "2":
									$value = (float)$cell->v;
								break;
							...
							case "4":
								...
								break;
							case "9":
									$value = round((float)$cell->v*100,0).'%';
								break;
							case "10":
									$value = round((float)$cell->v*100,2).'%';
								break;
							case "11":
									$decimal = (float)$cell->v;
									$split = explode('.', $decimal);
									$numAfterPoint = strlen($split[1]);
									$value = round((float)$cell->v,2).'E+'.$numAfterPoint;
								break;
							case "14":
									$value = $cell->v;
									$value = date('d-m-Y', mktime(0,0,0,1,$value-1,1900));
								break;
							case "15":
									$value = $cell->v;
							...
							case "16":
									$value = $cell->v;
									$value = date('d-M', mktime(0,0,0,1,$value-1,1900));
								break;
							case "17":
									$value = $cell->v;
								break;
					...
					if ( $this->workbook = $this->getEntryXML( $rel['Target'] )) {
						...
						if ( $workbookRelations = $this->getEntryXML( dirname($rel['Target']) . '/_rels/workbook.xml.rels' )) {
							...
							foreach ($workbookRelations->Relationship as $workbookRelation) {
								...
								} else if ($workbookRelation['Type'] == SimpleXLSX::SCHEMA_STYLES) {
									if ( $styles = $this->getEntryXML( $path ) ) {
											$this->styles = $styles;
									}
								}
							}
